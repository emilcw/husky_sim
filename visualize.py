...
# Base path for experiment data - relative to script location
script_dir = os.path.dirname(os.path.abspath(__file__))
experiment_data_base = os.path.join(script_dir, "experiment_data")
        ...
        for mode in ["dynamic"]:
            for planner in planners:
                for iteration in range(N):
                    file_path = os.path.join(experiment_data_base, f"{planner}_{world}_{mode}_{iteration}", file)
...
def plot_coverage_percent(world, data_frames, interval_dfs):
    ...
    for i, df_key in enumerate(data_frames):
        ...
        # Collision data - check if interval data exists and is not empty
        if df_key in interval_dfs and len(interval_dfs[df_key]) > 0:
            try:
                start_col = interval_dfs[df_key]["start-time"]
                end_col = interval_dfs[df_key]["end-time"]
            except KeyError:
                print(f"Warning: Missing columns in intervals.csv for {df_key}")
                start_col = pd.Series(dtype=float)
                end_col = pd.Series(dtype=float)
        else:
            start_col = pd.Series(dtype=float)
            end_col = pd.Series(dtype=float)
        ...
        # Only plot collision segments if there are collisions
        if len(start_col) > 0:
        ...
        for mode in modes:
            for planner in planners:
                ...
                if best_df is not None:
                    best_simulation_path = os.path.join(experiment_data_base, f"{planner}_{world}_{mode}_{best_simulation}", "intervals.csv")
                    key = f"{mode}_{planner}"
                    best_dfs[key] = best_df
                    # Check if intervals.csv exists, otherwise create empty DataFrame
                    if os.path.isfile(best_simulation_path):
                        try:
                            interval_dfs[key] = pd.read_csv(best_simulation_path)
                        except Exception as e:
                            print(f"Warning: Could not read intervals.csv for {key}: {e}")
                            # Create empty DataFrame with expected columns
                            interval_dfs[key] = pd.DataFrame(columns=["start-time", "end-time"])
                    else:
                        print(f"Warning: intervals.csv not found for {key}, creating empty DataFrame")
                        interval_dfs[key] = pd.DataFrame(columns=["start-time", "end-time"])
    ...
    for world in worlds:
        for planner in planners:
            ...
            for mode in modes:
                ...
                if best_df is not None:
                    file_path = os.path.join(experiment_data_base, f"{planner}_{world}_{mode}")
                    key = f"{mode}_{planner}"
                    best_dfs[key] = best_df
        ...
        for world in worlds:
            for mode in ["dynamic"]:
                for iteration in range(N):
                    ...
                    coverage_path = os.path.join(experiment_data_base, f"{planner}_{world}_{mode}_{iteration}", coverage_file)
                    if os.path.isfile(coverage_path): #Check if file exists  
                        try:
                            df = pd.read_csv(coverage_path)
                            total_coverages.append(df[" Coverage (%)"].iloc[-1])
                        except Exception as e:
                            print(f"Warning: Could not read {coverage_path}: {e}")
                        
                    # Logfile data
                    log_file = files[2]
                    log_file_path = os.path.join(experiment_data_base, f"{planner}_{world}_{mode}_{iteration}", log_file)
                    ...
                    collision_path = os.path.join(experiment_data_base, f"{planner}_{world}_{mode}_{iteration}", collision_file)
                    ...
                    if os.path.isfile(collision_path): #Check if file exists          
                        try:
                            ...
                            print(f"Warning: Could not read {collision_path}: {e}")
...
if __name__ == '__main__':
    ...
    if not os.path.exists(data_path + "plots/coverage_avg"):
        os.makedirs(data_path + "plots/coverage_avg")
    if not os.path.exists(data_path + "plots/coverage_p"):
        os.makedirs(data_path + "plots/coverage_p")
    if not os.path.exists(data_path + "plots/coverage_v"):
        os.makedirs(data_path + "plots/coverage_v")
    if not os.path.exists(data_path + "plots/collision"):
        os.makedirs(data_path + "plots/collision")
